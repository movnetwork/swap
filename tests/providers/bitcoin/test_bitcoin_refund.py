...
def test_bitcoin_refund():
    ...
    assert refund_signature.fee == signed_refund_transaction.fee == 576
    assert refund_signature.hash() == signed_refund_transaction.hash() == \
    ...
    assert refund_signature.json() == signed_refund_transaction.json()

    signed_refund_raw = refund_signature.signed_raw()
    assert signed_refund_raw == \

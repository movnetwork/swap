...
contract HTLC_XRC20 {
    ...
    modifier is_token_transferable (address token, address sender, uint256 amount) {
        ...
        require(XRC20(token).allowance(sender, address(this)) >= amount, "token allowance must be >= amount");
        _;
    }
    ...
    /**
     * @dev Sender sets up a new Hash Time Lock Contract (HTLC) and depositing the XRC20 token.
     *
     * @param token XRC20 Token contract address.
     * @param secret_hash A sha256 secret hash.
     * @param recipient Recipient account of the XRC20 token.
     * @param sender Sender account of the XRC20 token.
    ...
    function fund (
        ...
        if (!XRC20(token).transferFrom(sender, address(this), amount))
    ...
    function withdraw (bytes32 locked_contract_id, string memory preimage) external is_locked_contract_exist (locked_contract_id) check_secret_hash_matches (locked_contract_id, preimage) withdrawable (locked_contract_id) returns (bool) {
        ...
        XRC20(locked_contract.token).transfer(
    ...
    /**
     * @dev Get HTLC XRC20 contract details.
     *
     * @param locked_contract_id of HTLC XRC20 to get details.
     *
     * @return id token secret_hash recipient sender endtime amount withdrawn refunded preimage locked HTLC XRC20 contract data's.
     */

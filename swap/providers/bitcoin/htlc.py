...
from .utils import (
    script_from_address, is_address
)
...
class HTLC:
    ...
    def build(self, secret_hash: str, recipient_address: str,
        ...
        >>> htlc.build(secret_hash="3a26da82ead15a80533a02696656b14b5dbfd84eb14790f2e1be5e9e45820eeb", recipient_address="muTnffLDR5LtFeLR2i3WsKVfdyvzfyPnVB", sender_address="mphBPZf15cRFcL5tUq6mCbE84XobZ1vg7Q", sequence=1000)
        ...
        if not is_address(recipient_address, self._network):
            raise AddressError(f"Invalid recipient '{recipient_address}' {self._network} address.")
        if not is_address(sender_address, self._network):
            raise AddressError(f"Invalid sender '{sender_address}' {self._network} address.")

        self._script = IfElseScript(
            Hashlock256Script(
                hashlib.sha256(unhexlify(secret_hash)).digest(),
                script_from_address(
                    address=recipient_address, network=self._network)
            ),
            RelativeTimelockScript(
                Sequence(sequence),
                script_from_address(
                    address=sender_address, network=self._network)
            )
        )
        return self
    ...
    def from_bytecode(self, bytecode) -> _HTLC:
        """
    ...
    def bytecode(self) -> str:
        ...
        >>> from swap.providers.bitcoin.htlc import HTLC
        >>> htlc = HTLC(network="testnet")
        >>> htlc.build("3a26da82ead15a80533a02696656b14b5dbfd84eb14790f2e1be5e9e45820eeb", "muTnffLDR5LtFeLR2i3WsKVfdyvzfyPnVB", "mphBPZf15cRFcL5tUq6mCbE84XobZ1vg7Q", 1000)
        >>> htlc.bytecode()
